 ...
 * @since  2016
 * @see <https://github.com/colorcube/typo3_convert_extension_to_namespaces/>
 */
class convert_extension_to_namespaces {

    // don't do any string replace in these directories
    const DIR_EXCLUDE = array('.git', 'node_modules');
...
Review the changed files!\n";
    ...
    {
        echo "\nusage: " . basename($_SERVER['argv'][0]) . " path/to/my_ext\n";
    }
     ...
     */
    protected function convertExtension($source)
    {
        $source = rtrim($source, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR;

        echo "Convert Extension: {$source}\n\n";

        $search = array_keys(static::getMapArray());
        $replace = array_values(static::getMapArray());

        $this->copyAllFilesWithRename($source, $search, $replace);
        echo "\n";
     ...
     * @param array $replace
     */
    protected function copyAllFilesWithRename($from, $search, $replace)
    {
        $handle = opendir($from);
        if (!$handle) {
        ...
        while (false !== ($file = readdir($handle))) {
            ...
            // this directory shouldn't be processed
            if (is_dir($from . $file) && in_array($file, static::DIR_EXCLUDE)) {
                continue;
            ...
            // let's process the sub dir
                ...
                if ($file_parts['extension'] == 'php') {
                    ...
                    if ($fileContent != $fileContentNew) {
                        file_put_contents($from . $file, $fileContentNew);
                        file_put_contents($from . $file . '.bak', $fileContent);
                        echo "Modified {$from}{$file}\n";
                    }
                }
            }
        }
    ...
    /**
     * @return array guess what it is
     */
    protected static function getMapArray()
    {
        return array(
            ...
            "LLL:EXT:cms/locallang_ttc.xlf:bodytext.W.RTE" => "LLL:EXT:frontend/Resources/Private/Language/locallang_ttc.xlf:bodytext.W.RTE",
            "'script' => 'wizard_rte.php'" => "'module' => array('name' => 'wizard_rte')",

            // are these still used?
            "'script' => 'wizard_add.php'" => "'module' => array('name' => 'wizard_add')",
            "'script' => 'wizard_list.php'" => "'module' => array('name' => 'wizard_list')",
            "'script' => 'wizard_edit.php'" => "'module' => array('name' => 'wizard_edit')",


            // preprocess
            't3lib_div::intInRange' => 't3lib_utility_Math::forceIntegerInRange',
            'GeneralUtility::intInRange' => 'MathUtility::forceIntegerInRange',
...
$cli = new convert_extension_to_namespaces;